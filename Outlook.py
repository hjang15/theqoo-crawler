...
# === Outlook 메일 발송 ===
def send_outlook_email(subject, html_body):
    sender = os.environ.get("OUTLOOK_SENDER")
    receivers = os.environ.get("OUTLOOK_RECEIVER")  # 쉼표로 여러 주소 가능
    password = os.environ.get("OUTLOOK_PASSWORD")

    if not sender or not receivers or not password:
        print("❌ Outlook credentials are missing!")
        sys.exit(1)
...
def main():
    ...
    send_outlook_email(f"[크롤링]더쿠 게시글 크롤링 결과 - {today_str}", html_body)
